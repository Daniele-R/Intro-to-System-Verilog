...
void led_from_tapping(double distance, GpoCore *led_p) 
{
    uint16_t tap2led_map = 0x0000;                      // Reset LED map
    int num_leds = (int)(distance / 0.1);               // Calculate the number of LEDs to light (1 LED per 0.5m)
    
    if (num_leds > 16) 
    {
        num_leds = 16;                                  // Cap the number of LEDs to the maximum available (16 on Nexys4 DDR)
    }
    
    for (int i = 0; i < num_leds; i++) 
    {
        tap2led_map |= (uint16_t)(0x0001 << i);         // Set bits corresponding to the number of LEDs
    }
    
    led_p->write(tap2led_map);                          // Write the LED pattern to the FPGA
}
...
void spectrum_set(PwmCore *pwm_p, double distance) 
{
    pwm_p->set_freq(50);                                // Set PWM frequency
    double dist_adj = distance * 5.0;                   // Scale distance for spectrum logic (0.2m -> 1.0 increment)
    
    // Bit 0 is blue, bit 1 is green, bit 2 is red (check Nexys xdc file)
    if (dist_adj >= 0.0 && dist_adj <= 1) 
    {
        pwm_p->set_duty(1.0, 2);                         // Red
        pwm_p->set_duty(map_to_duty(dist_adj, 0.0, 1.0), 1);        // Fade into green
        pwm_p->set_duty(0.0, 0);                         // Blue off
    } 
    else if (dist_adj > 1 && dist_adj <= 2) 
    {
        pwm_p->set_duty(map_to_duty(2.0 - dist_adj, 1.0, 2.0), 2);  // Red fades out
        pwm_p->set_duty(1.0, 1);                         // Green
        pwm_p->set_duty(0.0, 0);                         // Blue off
    } 
    else if (dist_adj > 2 && dist_adj <= 3) 
    {
        pwm_p->set_duty(0.0, 2);                         // Red off
        pwm_p->set_duty(1.0, 1);                         // Green
        pwm_p->set_duty(map_to_duty(dist_adj - 2.0, 2.0, 3.0), 0);  // Fade into blue
    } 
    else if (dist_adj > 3 && dist_adj <= 4) 
    {
        pwm_p->set_duty(0.0, 2);                         // Red off
        pwm_p->set_duty(map_to_duty(4.0 - dist_adj, 3.0, 4.0), 1);  // Green fades out
        pwm_p->set_duty(1.0, 0);                         // Blue
    } 
    else if (dist_adj > 4 && dist_adj <= 5) 
    {
        pwm_p->set_duty(map_to_duty(dist_adj - 4.0, 4.0, 5.0), 2);  // Red fades in
        pwm_p->set_duty(0.0, 1);                         // Green off
        pwm_p->set_duty(1.0, 0);                         // Blue
    } 
    else if (dist_adj > 5) 
    {
        pwm_p->set_duty(1.0, 2);                         // Red
        pwm_p->set_duty(0.0, 1);                         // Green off
        pwm_p->set_duty(map_to_duty(6.0 - dist_adj, 5.0, 6.0), 0);  // Blue fades out
    }
}

void spectrum_set2(PwmCore *pwm_p, double distance) 
{
	pwm_p->set_freq(50);
	double volt_adj = distance*5;
	//bit 0 is blue, bit 1 is green, bit 2 is red (check Nexys xdc file)
	...
	if(volt_adj > 5.0 && volt_adj <= 6.0)
    {
		pwm_p->set_duty(1.0,2);
		pwm_p->set_duty(0.0,1);
		pwm_p->set_duty(map_to_duty(5.0 + 6.0 - distance, 5.0, 6.0),2);
	}
}
...
int main() 
{
    ...
    { 
        ...
        {
            display_double(&sseg, dist);        // the one presented in class
        }

        if (switch_state & 0x02) 
        {
            spectrum_set(&pwm, dist);
        }

        if (switch_state & 0x04) 
        {
            spectrum_set2(&pwm, dist);      // the one presented in class
        }

        if (switch_state & 0x08) 
        {
            led_from_tapping(dist, &led);
        }
        sleep_ms(50);
    }
}
