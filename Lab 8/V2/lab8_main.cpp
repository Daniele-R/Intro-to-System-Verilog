...
int get_delay_adc(XadcCore *adc_p, int adc_select)
{
	...
	//uart.disp(n);
	//uart.disp(" / ");
	adc_read = adc_p->read_adc_in(adc_select);
	//uart.disp(adc_read, 3);
	double delay = max_delay * adc_read;
	return (int)delay;
}
...
void led_chaser_2modes(GpiCore *sw_p, GpoCore *led_p, int max_led, XadcCore *adc_p, int adc_select)
{
   // Initialization
   ...
   mode_prev = mode;
   if(!(mode))
	   delay = get_delay_adc(adc_p, 0);
   else
	   delay = get_delay_sw(sw_p);
   ...
   if(!(sw_bit(sw_p, 0)))
   {
	   ...
	   for (i = 1; i < max_led; i++)
       {
          delay_prev = delay;
          mode_prev = mode;
          // When sw0 is switched from 0 to 1 in the middle
          if(sw_bit(sw_p, 0)){
        	  debug("state change: switch0 / up time", sw_bit(sw_p,0), now_ms());
        	  break;
          }

          led_p->write(1, i); //Toggle On
          sleep_ms(delay);
          led_p->write(0, i); //Toggle Off
          sleep_ms(delay);

          //Check mode
          mode = sw_bit(sw_p, 15);
          //Update delay based on mode
          if(!(mode))
       	   delay = get_delay_adc(adc_p, 0);
          else
       	   delay = get_delay_sw(sw_p);
          //Display if mode is changed
          if(mode != mode_prev)
          {
       	   uart.disp("mode: ");
       	   if(!(mode))
          ...
          //Display if change in delay detected
          if(delay != delay_prev)
            uart_delay(delay);
       }
...
//      adsr_check(&adsr, &led, &sw);
	   led_chaser_2modes(&sw, &led, 16, &adc, 0);
   } //while
} //main
