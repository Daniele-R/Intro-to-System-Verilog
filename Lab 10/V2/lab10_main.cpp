...
//   uart.disp("\n\r");
   ...
   mag_change = mag - mag_old;
   if(mag_change < 0)
	   mag_change = - mag_change;

   // Value was determined based on idle state?
   if(mag_change > 0.032) 
   {
	   uart.disp("magnitude change: ");
	   uart.disp(mag_change,3);
	   uart.disp("\n\r");
   }
...
int map_to_tap_strength(int min_tap, int max_tap, double in, double in_min, double in_max) 
{
	return (int)((in - in_min) * (double)(max_tap - min_tap) / (in_max - in_min)) + min_tap;
}

int get_tap_strength(SpiCore *spi_p) 
{
   const uint8_t RD_CMD = 0x0b;
   const uint8_t PART_ID_REG = 0x02;
   const uint8_t DATA_REG = 0x08;
   ...
   // Min and max values were determined through tapping
   double min_mag_change = 0.047;
   double thresh0 = 1.1;
   double thresh1 = 1.6;
   if(mag_change <= min_mag_change)
   {
	   tap_strength = 0;
   }
   if(mag_change > min_mag_change && mag_change <= thresh0) 
   {
	   tap_strength = map_to_tap_strength(1, 15, mag_change, min_mag_change, thresh0);
   }
   if(mag_change > thresh0 && mag_change <= thresh1) 
   {
	   tap_strength = map_to_tap_strength(15, 16, mag_change, thresh0, thresh1);
   }
   if(mag_change > thresh1) 
   {
	   tap_strength = 16;
   }
   //if(mag_change > min_mag_change){
	   //map_to_tap_strength(0, 16, mag_change, min_mag_change, thresh0);
	   //uart.disp("\"magnitude change\": ");
	   //uart.disp(mag_change, 3);
	   //uart.disp("\n\r");
	   //uart.disp("tap strength: ");
	   //uart.disp(tap_strength);
	   //uart.disp("\n\r");
   //}
...
{
	int tap_strength = get_tap_strength(spi_p);
	uint16_t tap2led_map = 0x0000;
	if(tap_strength > 0)
    {
		for(int i = 0; i < tap_strength;i++)
      {
			tap2led_map = tap2led_map | (uint16_t)(0x0001 << i);
		}
	}
	led_p->write(tap2led_map);
}
