...
void adc_check(XadcCore *adc_p, GpoCore *led_p, int n) {
   double reading;
   int n, i;
   uint16_t raw;
...
void adc_chasing_led(XadcCore *adc_p, GpoCore *led_p)
{
    ...
    static int time = 0; //wait time

    int old_time = time; //for sensing change in voltage
    time = 124 * reading + 138; //more voltage means slower speed
    ...
    if(old_time != time)
    {
        ...
        uart.disp(reading, 3);
        uart.disp("\n\r");
        uart.disp("current speed: "); //should really say current wait time
        uart.disp(time);
        uart.disp("\n\r");
    }
...
void spec_disp(XadcCore *adc_p, SsegCore *sseg_p, PwmCore *pwm_p)
{
    double reading = adc_p->read_adc_in(0);
    ...
    if(reading >= 0 && reading < 0.5/3.0)
    {
        pwm_p->set_duty(cap, 2);
        pwm_p->set_duty(cap, 2 + 3);
        pwm_p->set_duty(map(0,0.5/3.0, 0, cap, reading), 1);
        pwm_p->set_duty(map(0,0.5/3.0, 0, cap, reading), 1 + 3);
        pwm_p->set_duty(0, 0);
        pwm_p->set_duty(0, 0 + 3);
    }
    else if(reading >= 0.5/3.0 && reading < 1.0/3.0)
    {
        pwm_p->set_duty(map(0.5/3.0, 1.0/3.0, cap, 0, reading - 0.5/3.0), 2);
        pwm_p->set_duty(map(0.5/3.0, 1.0/3.0, cap, 0, reading - 0.5/3.0), 2 + 3);
        pwm_p->set_duty(cap, 1);
        pwm_p->set_duty(cap, 1 + 3);
        pwm_p->set_duty(0, 0);
        pwm_p->set_duty(0, 0 + 3);
    }
    else if(reading >= 1.0/3.0 && reading < 0.5)
    {
        pwm_p->set_duty(0, 2);
        pwm_p->set_duty(0, 2 + 3);
        pwm_p->set_duty(cap, 1);
        pwm_p->set_duty(cap, 1 + 3);
        pwm_p->set_duty(map(1.0/3.0, 0.5, 0, cap, reading - 1.0/3.0), 0);
        pwm_p->set_duty(map(1.0/3.0, 0.5, 0, cap, reading - 1.0/3.0), 0 + 3);
    }
    else if(reading >= 0.5 && reading < 2.0/3.0)
    {
        pwm_p->set_duty(0, 2);
        pwm_p->set_duty(0, 2 + 3);
        pwm_p->set_duty(map(0.5, 2.0/3.0, cap, 0, reading - 0.5), 1);
        pwm_p->set_duty(map(0.5, 2.0/3.0, cap, 0, reading - 0.5), 1 + 3);
        pwm_p->set_duty(cap, 0);
        pwm_p->set_duty(cap, 0 + 3);
    }
    else if(reading >= 2.0/3.0 && reading < 5.0/6.0)
    {
        pwm_p->set_duty(map(2.0/3.0, 5.0/6.0, 0, cap, reading - 2.0/3.0), 2);
        pwm_p->set_duty(map(2.0/3.0, 5.0/6.0, 0, cap, reading - 2.0/3.0), 2 + 3);
        pwm_p->set_duty(0, 1);
        pwm_p->set_duty(0, 1 + 3);
        pwm_p->set_duty(cap, 0);
        pwm_p->set_duty(cap, 0 + 3);
    }
    else if(reading >= 5.0/6.0) //till 1v
    {
        pwm_p->set_duty(cap, 2);
        pwm_p->set_duty(cap, 2 + 3);
        pwm_p->set_duty(0, 1);
        pwm_p->set_duty(0, 1 + 3);
        pwm_p->set_duty(map(5.0/6.0, 1, cap, 0, reading - 5.0/6.0), 0);
        pwm_p->set_duty(map(5.0/6.0, 1, cap, 0, reading - 5.0/6.0), 0 + 3);
    }    
    ...
    sseg_p->write_1ptn(sseg_p->h2s((int)reading), 3);
    sseg_p->write_1ptn(sseg_p->h2s((int)((reading - (int)reading)*10)), 2);
    sseg_p->write_1ptn(sseg_p->h2s((int)((reading*10 - (int)(reading*10))*10)), 1);
    sseg_p->write_1ptn(sseg_p->h2s((int)((reading*100 - (int)(reading*100))*10)), 0);
    sseg_p->set_dp(0x08);
}

//note: this function assumes y1 is the intercept as well since it is always the case for lab 9
float map(float x1, float x2, float y1, float y2, float x)
{
